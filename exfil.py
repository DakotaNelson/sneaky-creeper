...
class Exfil():
    ...
    def __init__(self, channel_name, encoder_names):
        c_class = self.__import_module('channels', channel_name)
        for encoder in encoder_names:
            e_class = self.__import_module('encoders', encoder.lower())
    ...
    @staticmethod
    def encoders():
        return ', '.join(encodingOptions)

    @staticmethod
    def channels():
        return ', '.join(channelOptions)
